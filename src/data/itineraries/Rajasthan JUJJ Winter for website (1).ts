
export const spitiTrekItinerary = {
  id: 'lw13',
  title: '"Colours of Rajasthan: A Journey Through Heritage & Sand"',
  location: 'Rajasthan',
  price: 23200,
  discount: 0,
  duration: '8 Days / 7 Nights',
  description: 'Discover the royal charm of Rajasthan with a journey through Jaipur, Udaipur, Jodhpur, and Jaisalmer — featuring majestic palaces, sacred temples, tranquil lakes, and thrilling desert adventures.',
  coverImage: 'https://images.unsplash.com/photo-1472396961693-142e6e269027?auto=format&fit=crop&w=1200&q=80',
  rating: 4.7,
  reviews: 89,
  images: [
    'https://images.unsplash.com/photo-1697984286841-5cedb7885109?q=80&w=1335&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'https://images.unsplash.com/photo-1628782379401-4fff9cdcbbfe?q=80&w=2340&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'https://images.unsplash.com/photo-1698753935121-153a106616d5?q=80&w=1287&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'https://images.unsplash.com/photo-1472396961693-142e6e269027?auto=format&fit=crop&w=1200&q=80'
  ],
  placesCovered: ['Jaipur', 'Udaipur', 'Jodhpur', 'Jaisalmer'],
  highlights: [
    'Explore Jaipur – Mandirs Bazaars & Iconic Landmarks',
    'Majestic full day Jaipur Sightseeing tour',
    'A unique and picturesque en route attraction - Marble Dumping Yard & Brahma Temple in Pushkar',
    'Udaipur - The City of Lakes, enjoy a romantic boat cruise on Lake Pichola',
    'Cultural Detour at Ranakpur Jain Temple',
    'Discover Jodhpur – Mehrangarh Fort, Umaid Bhavan Palace & Jaswant Thada',
    'Golden Glory of Jaisalmer - Forts Havelis & Lakes',
    'Camel Safari in Thar Desert – Ride into golden dunes and enjoy a magical sunset',
  ],
  inclusions: [
    'Accommodation on triple sharing in listed or similar category hotels',
    'Daily breakfast & dinner as per itinerary',
    'All land transportation (pick-up, drop-off & sightseeing)',
    'All vehicle expenses: fuel, tolls, parking, taxes & driver allowance',
    'Camel safari in Jaisalmer (2 pax per camel)',
    'Evening cultural program with snacks at Jaisalmer desert camp'
  ],
  exclusions: [
    'Lunch expenses',
    'Train Fares/Air Fares not included',
    'Monument entry fees, guide & camera charges',
    'All beverages (alcoholic, mineral water, soft drinks, etc.)',
    'Personal expenses: tips, calls, internet, laundry, etc.',
    'Charges for activities like pony rides, safaris, rafting, cable cars, etc.',
    'Extra food/drinks outside of buffet or fixed menu',
    'Vehicle use limited to allowed parking/government zones',
    'Costs due to delays from weather, health issues, or roadblocks',
    'Anything not mentioned under “Cost Includes”',
    'Cost of Travel Insurance',
    '5% GST not included in the package cost'
  ],
  faqs: [
    {
      question: 'What is the difficulty level of this trek?',
      answer: 'This is a moderate to challenging trek requiring good physical fitness. You\'ll be trekking at altitudes between 3,200m to 4,500m with daily walks of 4-6 hours.'
    },
    {
      question: 'How do we handle altitude sickness?',
      answer: 'Our itinerary includes proper acclimatization days. We carry first aid and oxygen cylinders. Gradual ascent and staying hydrated are key to preventing altitude sickness.'
    },
    {
      question: 'What should I pack for this trek?',
      answer: 'Essential items include warm clothes (temperature can drop below freezing), trekking boots, sleeping bag, sun protection, personal medications, and quick-dry clothing.'
    }
  ],
  itinerary: [
    {
      day: 1,
      title: 'Jaipur Pick Up',
      description: 'Begin your Royal Rajasthan journey in Jaipur with a warm welcome.',
      activities: [
        'Arrival & hotel transfer by local representative',
        'Visit the serene Birla Mandir',
        'Explore Jaipur’s colorful bazaars',
        
      ],
      meals: ['Dinner'],
      accommodation: 'Overnight Stay in Jaipur'
    },
    {
      day: 2,
      title: 'Jaipur Sightseeing',
      description: 'After breakfast, enjoy a full-day Jaipur city tour.',
      activities: [
        'Visit - ',
        'Amber Fort',
        'Jal Mahal',
        'Hawa Mahal',
        'Jantar Mantar',
        'City Palace',
        
      ],
      meals: ['Bfeakfast', 'Dinner'],
      accommodation: 'Enjoy your stay at Jaipur'
    },
    {
      day: 3,
      title: 'Jaipur To Udaipur',
      description: 'Explore the beauty of Kishangarh & Pushkar.',
      activities: [
        'Visit to the Marble Dumpimg Yard',
        'Visit Brahma Temple at Pushkar (if time permits)',
        'Transfer to Udaipur',
      
      ],
      meals: ['Breakfast', 'Dinner'],
      accommodation: 'Enjoy your stay at Udaipur'
    },
    {
      day: 4,
      title: 'Udaipur Sightseeing',
      description: 'Experience the royal charm of Udaipur with a romantic evening at lake Pichola.',
      activities: [
        'Visit scenic Fateh Sagar Lake',
        'Explore Saheliyon Ki Bari – Queen’s royal garden',
        'Discover the grandeur of City Palace – Rajasthan’s largest palace complex',
        'Seek blessings at Jagdish Temple – dedicated to Lord Vishnu',
        'Enjoy an evening boat cruise on Lake Pichola', 
      
      ],
      meals: ['Breakfast', 'Dinner'],
      accommodation: 'Enjoy your stay at Udaipur'
    },
    {
      day: 5,
      title: 'Udaipur To Jodhpur',
      description: 'En route to Jodhpur, visit the awe-inspiring Temple famed for it’s Marble Pillars & rich history.',
      activities: [
        'Enroute visit Ranakpur Jain Temple',
        'Admire its marble architecture & 1,444 carved pillars',
        'Spot the friendly monkey population around the temple',
        'Learn about the 90 ounces of gold donated by local women',
      
      ],
      meals: ['Breakfast', 'Dinner'],
      accommodation: 'Enjoy your stay at Jodhpur'
    },
    {
      day: 6,
      title: 'Jodhpur Sightseeing & Transfer to Jaisalmer',
      description: 'Enjoy full day of Jodhpur’s Beauty',
      activities: [
        'Umaid Bhavan Palace Museum',
        'Jaswant Thada',
        'Mehrangarh Fort (One of the largest fort build in 1459 by Rao Jodha',
        'Later, tranfer to Jiasalmer',
  
      ],
      meals: ['Breakfast', 'Dinner'],
      accommodation: 'Enjoy your stay at Jaisalmer'
    },
    {
      day: 7,
      title: 'Jaisalmer Sightseeing',
      description: 'Explore the golden charm of Jaisalmer & Discover its Deep Desert’s.',
      activities: [
        'Visit the iconic Jaisalmer Fort (Sonar Kella)',
        'Explore the historic Patwon-ki-Haveli with intricate carvings',
        'Relax at the scenic Gadisar Lake',
        'Experience a camel ride on Khudi Sand Dunes',
        'Witness a stunning Thar Desert sunset',

      ],
      meals: ['Breakfast', 'Dinner'],
      accommodation: 'Enjoy your stay at Jaisalmer'
    },
    {
      day: 8,
      title: 'Jaisalmer To Jodhpur Drop',
      description: 'End your journey with Bag full of Memories from the Land Of Royals.',
      activities: [
        'After breakfast transfer to Jodhpur railway station',
  
      ],
      meals: ['Breakfast'],
      accommodation: '-'
    }
  ],

  // Pricing tiers
  pricingOptions: [
    {
      id: 'standard',
      title: 'Without Train Fare',
      price: 23200,
      description: 'Package without Train Fare',
      isPopular: true
    },
    {
      id: 'deluxe',
      title: 'With Sleeper Train Fare',
      price: 24400,
      description: 'Package for Sleeper Train'
    },
    {
      id: 'premium',
      title: 'With A/C Train Fare',
      price: 26200,
      description: 'Package for A/C Train'
    }
  ],
  
  // Available batch dates
  batchDates: [
    {
      id: 'Batches On',
      startDate: '2025-09-27',
      endDate: '2025-10-06',
      totalSlots: 15,
      availableSlots: 15,
      price: 23200
    },
    {
      id: 'Batches On', 
      startDate: '2025-10-02',
      endDate: '2025-10-11',
      totalSlots: 15,
      availableSlots: 15,
      price: 23200
    },
    {
      id: 'Batches On', 
      startDate: '2025-10-18',
      endDate: '2025-10-27',
      totalSlots: 15,
      availableSlots: 15,
      price: 23200
    },
    {
      id: 'Batches On', 
      startDate: '2025-10-23',
      endDate: '2025-11-01',
      totalSlots: 15,
      availableSlots: 15,
      price: 23200
    },
    {
      id: 'Batches On', 
      startDate: '2025-11-01',
      endDate: '2025-11-10',
      totalSlots: 15,
      availableSlots: 15,
      price: 23200
    },
    {
      id: 'Batches On', 
      startDate: '2025-11-22',
      endDate: '2025-12-01',
      totalSlots: 15,
      availableSlots: 15,
      price: 23200
    }

    // Add more batch dates...
  ]
};
