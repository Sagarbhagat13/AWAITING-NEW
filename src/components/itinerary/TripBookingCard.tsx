...
const TripBookingCard = ({
  ...
  // Create pricing options for PricingTabs
  ...
  useEffect(() => {
    const defaultPricing = pricingOptions.find(option => option.id === activePricingId);
  ...
  const handleBatchDateSelect = (dateId: string | null) => {
    if (dateId) {
      const selected = batchDates.find(batch => batch.id === dateId);
